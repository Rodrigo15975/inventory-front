...
import { useGetAllMovements } from '@/modules/kardex/services/queriess.service'
import { format } from 'date-fns'
import { es } from 'date-fns/locale'
import { CalendarIcon, ChevronRight } from 'lucide-react'
import { useMemo, useState } from 'react'

export default function ProductManagementPageEntry() {
  const [page, setPage] = useState<number>(1)
  const [size, setSize] = useState<number>(30)
  const { data, isLoading } = useGetAllMovements(page, size)
  const [dateFilter, setDateFilter] = useState<Date | undefined>(undefined)
  const totalPages = Math.ceil((data?.count || 0) / size)
  const dataMovementsExits =
    data?.data?.filter((item) => item.moventType?.name === 'ENTRADA') || []

  const filteredData = useMemo(() => {
    if (!dataMovementsExits.length || !dateFilter)
      return { ...data, data: dataMovementsExits }

    const filterDate = dateFilter.toISOString().split('T')[0]

    return {
      ...data,
      data: dataMovementsExits.filter((item) => {
        const itemDate = new Date(item.createdAt).toISOString().split('T')[0]
        return itemDate === filterDate
      }),
    }
  }, [dataMovementsExits, dateFilter])
          ...
          <CardContent className="pt-6">
            <div className="flex justify-between items-center mb-4">
              <div className="text-xl font-semibold">
                ...
                <span className="text-[#16A34a]">Entrada</span>
              </div>
